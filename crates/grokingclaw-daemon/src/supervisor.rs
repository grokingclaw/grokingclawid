...
use grokingclaw_proxy::scope::ScopeConfig;
use grokingclaw_proxy::server::ProxyServer;
use grokingclaw_proxy::signer::RequestSigner;
...
impl Supervisor {
    ...
    pub async fn start_agent(&self, name: &str) -> Result<()> {
        ...
        let mut proxy_handle: Option<JoinHandle<()>> = None;
        ...
        if key_path.exists() {
            ...
            let scope = ScopeConfig::new(
                agent.config.allowed_domains.clone(),
            ...
            let (port, handle) = proxy.start().await?;
            ...
            tracing::warn!(
                agent = %name,
                "No agent.pem found, starting without proxy"
            );
        }
        ...
        agent.proxy_handle = proxy_handle;
        agent.status = AgentStatus::Running;
        agent.started_at = Some(Utc::now());

        tracing::info!(agent = %name, pid = ?pid, proxy = ?proxy_port, "Agent started");
        Ok(())
    }
    ...
    pub async fn stop_agent(&self, name: &str) -> Result<()> {
        ...
        if let Some(handle) = agent.proxy_handle.take() {
            tracing::info!(agent = %name, "Stopping sidecar proxy");
            handle.abort();
        }
