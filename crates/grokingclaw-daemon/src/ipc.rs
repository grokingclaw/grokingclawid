...
pub async fn start_ipc_server(socket_path: &Path, state: Arc<DaemonState>) -> Result<()> {
    ...
    match request.method.as_str() {
            ...
            match state.birth_agent(params).await {
                Ok(result) => {
                    ...
                    if let Value::Object(ref mut map) = value {
                        map.insert(
                            "birth_mode".to_string(),
                            serde_json::json!(result.birth_mode),
                        );
                        if let Some(cert) = &result.certificate {
                            map.insert(
                                "certificate_id".to_string(),
                                serde_json::json!(cert.certificate_id),
                            );
                        }
                    }
        ...
        "agents.start" => {
            ...
            match state.supervisor.start_agent(name).await {
                Ok(()) => JsonRpcResponse::success(
                    id,
                    serde_json::json!({"status": "started", "agent": name}),
                ),
        ...
        "agents.stop" => {
            ...
            match state.supervisor.stop_agent(name).await {
                Ok(()) => JsonRpcResponse::success(
                    id,
                    serde_json::json!({"status": "stopped", "agent": name}),
                ),
                Err(e) => JsonRpcResponse::error(id, -32000, format!("{:#}", e)),
            }
        }
        ...
        "agents.delete" => {
            ...
            match state.supervisor.delete_agent(name).await {
                Ok(()) => JsonRpcResponse::success(
                    id,
                    serde_json::json!({"status": "deleted", "agent": name}),
                ),
                Err(e) => JsonRpcResponse::error(id, -32000, format!("{:#}", e)),
            }
        }
        ...
        "mesh.connect" => {
            if let Some(ref mesh) = state.mesh {
                match mesh.connect().await {
                    Ok(()) => {
                        JsonRpcResponse::success(id, serde_json::json!({"status": "connected"}))
                    }
                    Err(e) => JsonRpcResponse::error(
                        id,
                        -32000,
                        format!("Mesh connection failed: {:#}", e),
                    ),
                }
            } else {
        ...
        "mesh.disconnect" => {
            if let Some(ref mesh) = state.mesh {
                match mesh.disconnect().await {
                    Ok(()) => {
                        JsonRpcResponse::success(id, serde_json::json!({"status": "disconnected"}))
                    }
                    Err(e) => JsonRpcResponse::error(
                        id,
                        -32000,
                        format!("Mesh disconnect failed: {:#}", e),
                    ),
                }
            } else {
                JsonRpcResponse::error(id, -32000, "Mesh networking is not enabled".to_string())
        ...
        "mesh.peers" => {
            if let Some(ref mesh) = state.mesh {
                match mesh.list_peers().await {
                    ...
                    Err(e) => {
                        JsonRpcResponse::error(id, -32000, format!("Failed to list peers: {:#}", e))
                    }
                }
            } else {
        ...
        // ─── Templates ─────────────────────────────────────────────
        "templates.list" => match state.templates.list_local() {
            Ok(templates) => {
            ...
            Err(e) => {
                JsonRpcResponse::error(id, -32000, format!("Failed to list templates: {:#}", e))
            }
        },
        ...
        "templates.inspect" => {
            ...
            match state.templates.get_template(name) {
                ...
                Ok(None) => {
                    JsonRpcResponse::error(id, -32001, format!("Template '{}' not found", name))
                }
                Err(e) => JsonRpcResponse::error(
                    id,
                    -32000,
                    format!("Failed to inspect template: {:#}", e),
                ),
            }
        }
        ...
        "templates.install" => {
            ...
            match state.templates.install_template(&name, &version).await {
                Ok(()) => JsonRpcResponse::success(
                    id,
                    serde_json::json!({"status": "installed", "template": name, "version": version}),
                ),
        ...
        "templates.create" => {
            ...
            if name.is_empty() || source.is_empty() {
                return JsonRpcResponse::error(
                    id,
                    -32602,
                    "Missing 'name' or 'source' parameter".to_string(),
                );
            }
            match state
                .templates
                .create_from_local(&name, &std::path::PathBuf::from(&source))
            {
                Ok(()) => JsonRpcResponse::success(
                    id,
                    serde_json::json!({"status": "created", "template": name}),
                ),
        ...
        "audit.query" => {
            ...
            match state.query_audit(name, last, verify).await {
                ...
                Err(e) => {
                    JsonRpcResponse::error(id, -32000, format!("Audit query failed: {:#}", e))
                }
            }
        }
        ...
        "update.check" => {
            ...
            if let Some(ref checker) = *uc {
                match checker.check().await {
                    ...
                    Err(e) => {
                        JsonRpcResponse::error(id, -32000, format!("Update check failed: {:#}", e))
                    }
                }
            } else {
                JsonRpcResponse::success(
                    id,
                    serde_json::json!({"available_updates": [], "message": "Update checker not initialized"}),
                )
            }
        }
        ...
        "update.apply" => {
            ...
            if let Some(ref checker) = *uc {
                match checker.apply_template_updates().await {
                    Ok(applied) => JsonRpcResponse::success(
                        id,
                        serde_json::json!({
                            "applied": applied,
                            "count": applied.len(),
                    ...
                    Err(e) => {
                        JsonRpcResponse::error(id, -32000, format!("Update apply failed: {:#}", e))
                    }
                }
            } else {
